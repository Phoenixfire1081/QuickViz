	...
	def setSlice1_actual(self, scNumber, figureHandle):
		...
		if self.sliceType == 'Streamlines':
			...
			if self.planeOrientation == 'X':
				...
				if scNumber == 1:
					self.volSlice1_sc1 = mlab.pipeline.image_plane_widget(self.sf1_sc1, plane_orientation = 'x_axes', \
					slice_index = self.whichSliceX, colormap = self.contourColormap1, opacity = self.contourOpacity1, \
					figure=figureHandle, vmin = self.colormapMin1, vmax = self.colormapMax1)
				if scNumber == 2:
					self.volSlice1_sc2 = mlab.pipeline.image_plane_widget(self.sf1_sc2, plane_orientation = 'x_axes', \
					slice_index = self.whichSliceX, colormap = self.contourColormap1, opacity = self.contourOpacity1, \
					figure=figureHandle, vmin = self.colormapMin1, vmax = self.colormapMax1)
				if scNumber == 3:
					self.volSlice1_sc3 = mlab.pipeline.image_plane_widget(self.sf1_sc3, plane_orientation = 'x_axes', \
					slice_index = self.whichSliceX, colormap = self.contourColormap1, opacity = self.contourOpacity1, \
					figure=figureHandle, vmin = self.colormapMin1, vmax = self.colormapMax1)
				if scNumber == 4:
					self.volSlice1_sc4 = mlab.pipeline.image_plane_widget(self.sf1_sc4, plane_orientation = 'x_axes', \
					slice_index = self.whichSliceX, colormap = self.contourColormap1, opacity = self.contourOpacity1, \
					figure=figureHandle, vmin = self.colormapMin1, vmax = self.colormapMax1)
				...
				if scNumber == 1:
					self.volSlice1_sc1 = mlab.pipeline.image_plane_widget(self.sf1_sc1, plane_orientation = 'y_axes', \
					slice_index = self.whichSliceY, colormap = self.contourColormap1, opacity = self.contourOpacity1, \
					figure=figureHandle, vmin = self.colormapMin1, vmax = self.colormapMax1)
				if scNumber == 2:
					self.volSlice1_sc2 = mlab.pipeline.image_plane_widget(self.sf1_sc2, plane_orientation = 'y_axes', \
					slice_index = self.whichSliceY, colormap = self.contourColormap1, opacity = self.contourOpacity1, \
					figure=figureHandle, vmin = self.colormapMin1, vmax = self.colormapMax1)
				if scNumber == 3:
					self.volSlice1_sc3 = mlab.pipeline.image_plane_widget(self.sf1_sc3, plane_orientation = 'y_axes', \
					slice_index = self.whichSliceY, colormap = self.contourColormap1, opacity = self.contourOpacity1, \
					figure=figureHandle, vmin = self.colormapMin1, vmax = self.colormapMax1)
				if scNumber == 4:
					self.volSlice1_sc4 = mlab.pipeline.image_plane_widget(self.sf1_sc4, plane_orientation = 'y_axes', \
					slice_index = self.whichSliceY, colormap = self.contourColormap1, opacity = self.contourOpacity1, \
					figure=figureHandle, vmin = self.colormapMin1, vmax = self.colormapMax1)
				...
				if scNumber == 1:
					self.volSlice1_sc1 = mlab.pipeline.image_plane_widget(self.sf1_sc1, plane_orientation = 'z_axes', \
					slice_index = self.whichSliceZ, colormap = self.contourColormap1, opacity = self.contourOpacity1, \
					figure=figureHandle, vmin = self.colormapMin1, vmax = self.colormapMax1)
				if scNumber == 2:
					self.volSlice1_sc2 = mlab.pipeline.image_plane_widget(self.sf1_sc2, plane_orientation = 'z_axes', \
					slice_index = self.whichSliceZ, colormap = self.contourColormap1, opacity = self.contourOpacity1, \
					figure=figureHandle, vmin = self.colormapMin1, vmax = self.colormapMax1)
				if scNumber == 3:
					self.volSlice1_sc3 = mlab.pipeline.image_plane_widget(self.sf1_sc3, plane_orientation = 'z_axes', \
					slice_index = self.whichSliceZ, colormap = self.contourColormap1, opacity = self.contourOpacity1, \
					figure=figureHandle, vmin = self.colormapMin1, vmax = self.colormapMax1)
				if scNumber == 4:
			...
			if self.planeOrientation == 'X':
				...
				if scNumber == 1:
					self.volSlice1_sc1 = mlab.pipeline.vector_cut_plane(self.sf1_sc1, plane_orientation = 'x_axes', \
					colormap = self.contourColormap1, opacity = self.contourOpacity1, scale_factor = self.scaleFactorSlice,\
					resolution = self.resolutionSlice, figure=figureHandle, vmin = self.colormapMin1, vmax = self.colormapMax1)
					self.volSlice1_sc1.implicit_plane.widget.origin = ((self.whichSliceX * self.dx_data1) + self.xmin_data1, 0, 0)
				if scNumber == 2:
					self.volSlice1_sc2 = mlab.pipeline.vector_cut_plane(self.sf1_sc2, plane_orientation = 'x_axes', \
					colormap = self.contourColormap1, opacity = self.contourOpacity1, scale_factor = self.scaleFactorSlice,\
					resolution = self.resolutionSlice, figure=figureHandle, vmin = self.colormapMin1, vmax = self.colormapMax1)
					self.volSlice1_sc2.implicit_plane.widget.origin = ((self.whichSliceX * self.dx_data1) + self.xmin_data1, 0, 0)
				if scNumber == 3:
					self.volSlice1_sc3 = mlab.pipeline.vector_cut_plane(self.sf1_sc3, plane_orientation = 'x_axes', \
					colormap = self.contourColormap1, opacity = self.contourOpacity1, scale_factor = self.scaleFactorSlice,\
					resolution = self.resolutionSlice, figure=figureHandle, vmin = self.colormapMin1, vmax = self.colormapMax1)
					self.volSlice1_sc3.implicit_plane.widget.origin = ((self.whichSliceX * self.dx_data1) + self.xmin_data1, 0, 0)
				if scNumber == 4:
					self.volSlice1_sc4 = mlab.pipeline.vector_cut_plane(self.sf1_sc4, plane_orientation = 'x_axes', \
					colormap = self.contourColormap1, opacity = self.contourOpacity1, scale_factor = self.scaleFactorSlice,\
					resolution = self.resolutionSlice, figure=figureHandle, vmin = self.colormapMin1, vmax = self.colormapMax1)
				...
				if scNumber == 1:
					self.volSlice1_sc1 = mlab.pipeline.vector_cut_plane(self.sf1_sc1, plane_orientation = 'y_axes', \
					colormap = self.contourColormap1, opacity = self.contourOpacity1, scale_factor = self.scaleFactorSlice,\
					resolution = self.resolutionSlice, figure=figureHandle, vmin = self.colormapMin1, vmax = self.colormapMax1)
					self.volSlice1_sc1.implicit_plane.widget.origin = (0, (self.whichSliceY * self.dy_data1) + self.ymin_data1, 0)
				if scNumber == 2:
					self.volSlice1_sc2 = mlab.pipeline.vector_cut_plane(self.sf1_sc2, plane_orientation = 'y_axes', \
					colormap = self.contourColormap1, opacity = self.contourOpacity1, scale_factor = self.scaleFactorSlice,\
					resolution = self.resolutionSlice, figure=figureHandle, vmin = self.colormapMin1, vmax = self.colormapMax1)
					self.volSlice1_sc2.implicit_plane.widget.origin = (0, (self.whichSliceY * self.dy_data1) + self.ymin_data1, 0)
				if scNumber == 3:
					self.volSlice1_sc3 = mlab.pipeline.vector_cut_plane(self.sf1_sc3, plane_orientation = 'y_axes', \
					colormap = self.contourColormap1, opacity = self.contourOpacity1, scale_factor = self.scaleFactorSlice,\
					resolution = self.resolutionSlice, figure=figureHandle, vmin = self.colormapMin1, vmax = self.colormapMax1)
					self.volSlice1_sc3.implicit_plane.widget.origin = (0, (self.whichSliceY * self.dy_data1) + self.ymin_data1, 0)
				if scNumber == 4:
					self.volSlice1_sc4 = mlab.pipeline.vector_cut_plane(self.sf1_sc4, plane_orientation = 'y_axes', \
					colormap = self.contourColormap1, opacity = self.contourOpacity1, scale_factor = self.scaleFactorSlice,\
					resolution = self.resolutionSlice, figure=figureHandle, vmin = self.colormapMin1, vmax = self.colormapMax1)
				...
				if scNumber == 1:
					self.volSlice1_sc1 = mlab.pipeline.vector_cut_plane(self.sf1_sc1, plane_orientation = 'z_axes', \
					colormap = self.contourColormap1, opacity = self.contourOpacity1, scale_factor = self.scaleFactorSlice,\
					resolution = self.resolutionSlice, figure=figureHandle, vmin = self.colormapMin1, vmax = self.colormapMax1)
					self.volSlice1_sc1.implicit_plane.widget.origin = (0, 0, (self.whichSliceZ * self.dz_data1) + self.zmin_data1)
				if scNumber == 2:
					self.volSlice1_sc2 = mlab.pipeline.vector_cut_plane(self.sf1_sc2, plane_orientation = 'z_axes', \
					colormap = self.contourColormap1, opacity = self.contourOpacity1, scale_factor = self.scaleFactorSlice,\
					resolution = self.resolutionSlice, figure=figureHandle, vmin = self.colormapMin1, vmax = self.colormapMax1)
					self.volSlice1_sc2.implicit_plane.widget.origin = (0, 0, (self.whichSliceZ * self.dz_data1) + self.zmin_data1)
				if scNumber == 3:
					self.volSlice1_sc3 = mlab.pipeline.vector_cut_plane(self.sf1_sc3, plane_orientation = 'z_axes', \
					colormap = self.contourColormap1, opacity = self.contourOpacity1, scale_factor = self.scaleFactorSlice,\
					resolution = self.resolutionSlice, figure=figureHandle, vmin = self.colormapMin1, vmax = self.colormapMax1)
					self.volSlice1_sc3.implicit_plane.widget.origin = (0, 0, (self.whichSliceZ * self.dz_data1) + self.zmin_data1)
				if scNumber == 4:
					self.volSlice1_sc4 = mlab.pipeline.vector_cut_plane(self.sf1_sc4, plane_orientation = 'z_axes', \
					colormap = self.contourColormap1, opacity = self.contourOpacity1, scale_factor = self.scaleFactorSlice,\
					resolution = self.resolutionSlice, figure=figureHandle, vmin = self.colormapMin1, vmax = self.colormapMax1)
